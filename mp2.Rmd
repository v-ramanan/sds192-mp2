...
```{r}
general_votes_plot <- ggplot(neg_money_2, aes(x = party, y = general_votes, fill = transaction_type)) + geom_col(position = position_dodge())
...
```{r}
incumbent_plot <- ggplot(win_lose, aes(x = party, y = general_votes, fill = incumbent) + geom_col(position = position_dodge()))
...
```{r}
win_lose <- contributions_fec %>%
  left_join(sum_money, by = "cmte_name") %>%
  filter(transaction_type== c("24A", "24E"), state.x == "NY") %>%
  select(party, incumbent) %>%
  summarise(N=n_distinct(), winners= sum(N))
