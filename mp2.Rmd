...
```{r}
win_lose <- cont_neg_final %>%
  left_join(sum_money, by = "cmte_name") %>%
  filter(transaction_type== c("24A", "24E"), state.x == "NY") %>%
  select(party, incumbent, transaction_type) %>%
  group_by(party, transaction_type) %>%
  summarize(N = n(), 
            total = sum(n), 
...
```{r}
incumbent_plot <- ggplot(win_lose, aes(x = party, y = , fill = transaction_type) + geom_col(position = position_dodge()))
...
```{r}
win_lose <- cont_neg_final %>%  left_join(sum_money, by = "cmte_name") %>%  filter(transaction_type== c("24A", "24E"), state.x == "NY") %>%  select(party, incumbent, transaction_type) %>%  group_by(party, transaction_type) %>%  summarize(N = n(),            total = sum(n),            boys = sum(ifelse(sex == "M", n, 0))) win_lose
```

```{r}
incumbent_plot <- ggplot(win_lose, aes(x = party, y = , fill = transaction_type) + geom_col(position = position_dodge()))                                                                                     incumbent_plot
```
