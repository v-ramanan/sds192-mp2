...
donations_df
#  doing basically the same thing as the data frame above but now groupnig by the transaction type. We want to see how much money was used both in favor of a party and against a party. 
```
...
donations_plot <- ggplot(donations_df, aes(x = party, y = new_sum, fill = transaction_type))+
  geom_col(position = position_dodge())+
     scale_x_discrete(label = c("Conservative","Democrat","Green", "Libertarian","Republican","Socialist", "Write In"))+
   scale_fill_manual(name="Transaction Type",values = c("#CC6666", "#9999CC"),labels=c("Money Used Against a party", "Money Used For a Party")) +theme(legend.position = "top")+
